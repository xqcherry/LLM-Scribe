...
| # | 方向 | 优先级 | 状态 | 一句话目标 |
|---|------|--------|------|-----------|
| 1 | 报告视觉重做 | 高 | 已完成 | 重排信息架构，群友视角，多主题可切换 |
| 2 | 摘要分块 + 滑动窗口 + 多级缓存 | 最高 | 已完成 | 激活多阶段摘要，建立缓存与增量机制，治本效果与成本 |
...
- [x] 1.1 定义设计 token 与基础样式系统
- [x] 1.2 重排统计区信息架构，技术指标移至页脚常显
- [x] 1.3 重做话题卡（参与度可视化、编号节奏、空状态）
- [x] 1.4 胶囊 HTML 结构迁移至模板，`generators.py` 只产数据
- [x] 1.5 多主题目录与 `templates.py` 回退逻辑修复
- [x] 1.6 浏览器实测渲染效果，对比改前改后
...
- [x] 2.1 新建 `infrastructure/cache/`，定义缓存端口与内存/可持久化实现
- [x] 2.2 graph 增加分块 + extract + compress_merge 节点，按消息量路由
- [x] 2.3 激活 `compression_prompt` / `extraction_prompt`，接入多阶段链路
- [x] 2.4 实现 L1 最终结果缓存（TTL 内直接命中，键为 group_id+hours）
- [x] 2.5 实现 L2 分块摘要缓存（块内容 hash）
- [x] 2.6 实现 L3 锚点状态存储 + 滑动窗口增量合并逻辑
- [x] 2.7 激活 `memory_context` 机制，喂入上次压缩摘要
- [x] 2.8 `db_connection` 改 `aiomysql` 连接池，repository 改异步
- [x] 2.9 `max_tokens` 按模型动态化
- [x] 2.10 修 `format_messages` 返回值在 `summary_chain` 被丢弃的问题
- [x] 2.11 端到端验证：长窗效果提升、重复请求命中缓存、增量请求只处理新消息
