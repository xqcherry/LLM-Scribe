...
from llm_scribe.Prompt.prompt2 import append_prompt
...
from llm_scribe.memory.semantic import semantic_extract, semantic_merge, semantic_filter
...
def high_refresh(group_id, msgs, hours):
    # 语义池部分
    blocks = semantic_extract(msgs)
    short = load_memory_short(group_id)
    old_pool = short.get("mem_json", {})
    pool = semantic_merge(old_pool, blocks)
    semantic_related = semantic_filter(pool, msgs)
    # 基础信息
    meta = base_info(msgs)
    # 摘要部分
    prompt = create_prompt(pool, msgs, meta, semantic_related)
...
def mid_refresh(group_id, new_msgs, short, cache):
    old_pool = short["mem_json"] # short片段是上次记忆
    old_summary = old_pool.get("last_summary", "")
    # 语义池部分
    blocks = semantic_extract(new_msgs)
    pool = semantic_merge(old_pool, blocks)
    semantic_related = semantic_filter(pool, cache["msgs"])
    # 生成新摘要: summary + increasing
    prompt = append_prompt(new_msgs, pool, semantic_related)
...
def low_refresh(group_id, msgs, short):
    ...
    if not msgs:
        # 本时间段没有消息 → 不更新 cache，不更新 last_summary
